...
public class GetMessageCode {
    ...
    public static Integer sendSms(String telephone,HfMessageInfo info, HfMessageTemplate template) throws ClientException{
        ...
        //必填:短信模板-可在短信控制台中找到
        request.setTemplateCode(template.getTemplateId());    // TODO 修改成自己的
        //可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
        String[] str = template.getTemplateParam().split(",");
        ...
        for (int i = 0; i < str.length; i++) {
			...
			builder.append(":");
			builder.append("\"");
			builder.append(findData(template.getType(),str[i]));
			builder.append("\"");
		}
    ...
    public static String findData(String type, String contentType) {
    	String result = "";
    	if("login".equals(type)) {
    		switch (contentType) {
			case "code":
				result = String.valueOf(setNewcode());
				break;
			default:
				break;
			}
